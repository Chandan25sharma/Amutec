...
export default function OrganizePDFPage() {
  ...
  const loadPDFPages = async (file: File) => {
    try {
      // Mock page loading - in real implementation, extract page previews
      const mockPages: PageInfo[] = Array.from({ length: 10 }, (_, i) => ({
        pageNumber: i + 1,
        preview: `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="100" height="140" viewBox="0 0 100 140">
          <rect width="100" height="140" fill="#f8f9fa" stroke="#dee2e6" stroke-width="1"/>
          <text x="50" y="70" text-anchor="middle" font-family="Arial" font-size="16" fill="#6c757d">Page ${i + 1}</text>
        </svg>`)}`
      }))
      setPages(mockPages)
    } catch (err) {
  ...
  const deletePages = () => {
    const newPages = pages.filter(page => !selectedPages.has(page.pageNumber))
    setPages(newPages)
      ...
      <div className="max-w-6xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600">
            Rearrange, rotate, or delete pages in your PDF document with an intuitive drag-and-drop interface.
          </p>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6 mb-8">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-gray-500">{pages.length} pages • {(file.size / 1024 / 1024).toFixed(2)} MB</p>
                </div>
                <button
                  onClick={() => {
                    setFile(null)
                ...
                {pages.map((page, index) => (
                  ...
                  >
                    ...
                    <div className="aspect-[3/4] bg-gray-100 rounded mb-2 flex items-center justify-center">
                      {page.preview ? (
                        <img src={page.preview} alt={`Page ${page.pageNumber}`} className="w-full h-full object-contain" />
                      ) : (
                        <span className="text-gray-400 text-sm">Page {page.pageNumber}</span>
                      )}
                    </div>

                    {/* Page Controls */}
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium text-gray-600">Page {page.pageNumber}</span>
        ...
        {downloadUrl && (
          <div className="bg-white rounded-xl shadow-sm border p-8">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 flex items-center">
              ...
              <span className="text-green-700">PDF organized successfully!</span>
            </div>
